    ...
    public class ShelterBehaviorManager : UpdatableAndDeletable, INotifyWhenRoomIsReady
    {
        ...
        public ShelterBehaviorManager(Room instance, PlacedObject pObj)
        {
            ...
            for (int i = 0; i < instance.roomSettings.placedObjects.Count; i++)
            {
                ...
                {
                    if (instance.roomSettings.placedObjects[i].type == SBehCentral.EnumExt_ShelterBehaviorsMod.ShelterBhvrPlacedDoor)
                    {
                        this.AddPlacedDoor(instance.roomSettings.placedObjects[i]);
                    }
                    else if (instance.roomSettings.placedObjects[i].type == SBehCentral.EnumExt_ShelterBehaviorsMod.ShelterBhvrTriggerZone)
                    {
                        this.AddTriggerZone(instance.roomSettings.placedObjects[i]);
                    }
                    else if (instance.roomSettings.placedObjects[i].type == SBehCentral.EnumExt_ShelterBehaviorsMod.ShelterBhvrNoTriggerZone)
                    {
